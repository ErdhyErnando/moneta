...
// Uses database-level aggregation with DATE_TRUNC for efficient month grouping
async function getMonthlyData(
	...
	// Group by calendar month of the stored timestamp
	const monthlyData = await db
		.select({
			month: sql<string>`TO_CHAR(DATE_TRUNC('month', ${table.date}), 'YYYY-MM-DD')`,
			total: sql<number>`COALESCE(SUM(CAST(${table.amount} AS DECIMAL)), 0)`,
		})
		.from(table)
		.where(and(...conditions))
		.groupBy(sql`DATE_TRUNC('month', ${table.date})`);
