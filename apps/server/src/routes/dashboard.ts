...
// Uses database-level aggregation with DATE_TRUNC for efficient UTC month grouping
async function getMonthlyData(
	table: typeof expenses | typeof incomes,
	userId: string,
	year: number,
) {
	// Define UTC boundaries for the year
	const startOfYear = new Date(Date.UTC(year, 0, 1, 0, 0, 0, 0));
	const endOfYear = new Date(Date.UTC(year, 11, 31, 23, 59, 59, 999));

	// Use DATE_TRUNC with 'month' to group by UTC month at the database level
	// This ensures the database does the heavy lifting and only returns aggregated rows
	const monthlyData = await db
		.select({
			// DATE_TRUNC('month', date AT TIME ZONE 'UTC') truncates to the first of each month in UTC
			month: sql<string>`TO_CHAR(DATE_TRUNC('month', ${table.date} AT TIME ZONE 'UTC'), 'YYYY-MM-DD')`,
			total: sql<number>`COALESCE(SUM(CAST(${table.amount} AS DECIMAL)), 0)`,
		})
		.from(table)
		.where(
		...
		.groupBy(sql`DATE_TRUNC('month', ${table.date} AT TIME ZONE 'UTC')`);

	// Transform to expected format (month as "YYYY-MM-01", amount as string)
